...
void TcpCubicSenderBytes::SetSlowstartthresholdInBytesPublic(QuicByteCount threshhold) {
  slowstart_threshold_ = std::max(min_congestion_window_,std::min(threshhold, slowstart_threshold_)); 
}

void TcpCubicSenderBytes::SetSlowstartthresholdFromBandwidthAndRttPublic(QuicBandwidth bandwidth,QuicTime::Delta rtt) {
  QuicByteCount threshhold = bandwidth.ToBytesPerPeriod(rtt);
  slowstart_threshold_ = std::max(min_congestion_window_,std::min(threshhold, slowstart_threshold_)); 
}

void TcpCubicSenderBytes::SetMinSlowstartExitWindowFromBandwidthAndRttPublic(QuicBandwidth bandwidth,QuicTime::Delta rtt){
  QuicByteCount windowSize = bandwidth.ToBytesPerPeriod(rtt);
  min_slow_start_exit_window_ = std::min(congestion_window_, std::max(initial_tcp_congestion_window_/2,windowSize));
}

void TcpCubicSenderBytes::SetMinSlowstartExitWindowPublic(QuicByteCount windowSize){
  min_slow_start_exit_window_ = std::min(congestion_window_, std::max(initial_tcp_congestion_window_/2,windowSize));
}

void TcpCubicSenderBytes::SetCongestionWindowFromBandwidthAndRttPublic(QuicBandwidth bandwidth,
                                            QuicTime::Delta rtt) {
...
void TcpCubicSenderBytes::SetCongestionWindowInPacketsPublic(QuicPacketCount congestion_window){
  congestion_window_ = congestion_window * kDefaultTCPMSS;
}

void TcpCubicSenderBytes::SetInitialCongestionWindowInBytesPublic(QuicByteCount congestion_window){
  initial_tcp_congestion_window_ = congestion_window;
}

void TcpCubicSenderBytes::SetInitialCongestionWindowFromBandwidthAndRttPublic(QuicBandwidth bandwidth, QuicTime::Delta rtt){
  QuicByteCount new_congestion_window = bandwidth.ToBytesPerPeriod(rtt);
  // Limit new CWND if needed.
...
void TcpCubicSenderBytes::OnPacketLost(QuicPacketNumber packet_number,
  ...
  // std::cout << "OnPacketLost" << std::endl;
...
void TcpCubicSenderBytes::MaybeIncreaseCwnd(
  ...
  // std::cout << initial_tcp_congestion_window_ << std::endl;
