...
class QUIC_EXPORT_PRIVATE RttStats {
 public:
  ...
  // Sets an initial smoothed RTT 
  void set_initial_srtt_us(int64_t initial_rtt_us) {
    if (initial_rtt_us <= 0) {
      QUIC_BUG << "Attempt to set initial smoothed rtt to <= 0.";
      return;
    }
    smoothed_rtt_ = QuicTime::Delta::FromMicroseconds(initial_rtt_us);
  }

   // Sets an initial min RTT 
  void set_initial_min_rtt_us(int64_t initial_min_rtt_us) {
    if (initial_min_rtt_us <= 0) {
      QUIC_BUG << "Attempt to set initial min rtt to <= 0.";
      return;
    }
    min_rtt_ = QuicTime::Delta::FromMicroseconds(initial_min_rtt_us);
  }
