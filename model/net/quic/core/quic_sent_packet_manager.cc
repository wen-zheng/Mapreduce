...
QuicSentPacketManager::QuicSentPacketManager(
    ...
    : unacked_packets_(),
      ...
      largest_packet_peer_knows_is_acked_(0) {
        // std::cout << congestion_control_type << std::endl;
...
void QuicSentPacketManager::SetFromConfig(const QuicConfig& config) {
  ...
  if (config.HasClientRequestedIndependentOption(kRENO, perspective_)) {
    if (config.HasClientRequestedIndependentOption(kBYTE, perspective_)) {
      ...
      // std::cout << "kRenoBytes" << std::endl;
    } else {
      SetSendAlgorithm(kReno);
      // std::cout << "KRENO" << std::endl;
    }
...
void QuicSentPacketManager::SetNumOpenStreams(size_t num_streams) {
  // std::cout << "SetNumOpenStreams" << std::endl;
...
void QuicSentPacketManager::UpdatePacketInformationReceivedByPeer(
  ...
  // std::cout << "UpdatePacketInformationReceivedByPeer" << std::endl;
...
void QuicSentPacketManager::MaybeInvokeCongestionEvent(
    ...
    QuicTime event_time) {
  // std::cout << "MaybeInvokeCongestionEvent" << std::endl;
...
void QuicSentPacketManager::HandleAckForSentPackets(
    const QuicAckFrame& ack_frame) {
  // std::cout << "HandleAckForSentPackets" << std::endl;
...
void QuicSentPacketManager::RetransmitUnackedPackets(
  ...
  // std::cout << "RetransmitUnackedPackets" << std::endl;
...
void QuicSentPacketManager::NeuterUnencryptedPackets() {
  // std::cout << "NeuterUnencryptedPackets" << std::endl;
  QuicPacketNumber packet_number = unacked_packets_.GetLeastUnacked();
  for (QuicUnackedPacketMap::const_iterator it = unacked_packets_.begin();
       it != unacked_packets_.end(); ++it, ++packet_number) {
    if (!it->retransmittable_frames.empty() &&
...
void QuicSentPacketManager::MarkForRetransmission(
  ...
  // std::cout << "MarkForRetransmission" << std::endl;
...
void QuicSentPacketManager::RecordOneSpuriousRetransmission(
  ...
  // std::cout << "RecordOneSpuriousRetransmission" << std::endl;
...
void QuicSentPacketManager::RecordSpuriousRetransmissions(
  ...
  // std::cout << "RecordSpuriousRetransmissions" << std::endl;
...
bool QuicSentPacketManager::HasPendingRetransmissions() const {
  // std::cout << "HasPendingRetransmissions" << std::endl;
  return !pending_retransmissions_.empty();
}

QuicPendingRetransmission QuicSentPacketManager::NextPendingRetransmission() {
  // std::cout << "NextPendingRetransmission" << std::endl;
...
QuicPacketNumber QuicSentPacketManager::GetNewestRetransmission(
  ...
  // std::cout << "GetNewestRetransmission" << std::endl;
...
void QuicSentPacketManager::MarkPacketHandled(QuicPacketNumber packet_number,
  ...
  // std::cout << "MarkPacketHandled" << std::endl;
...
bool QuicSentPacketManager::OnPacketSent(
  ...
  // std::cout << "OnPacketSent" << std::endl;
...
void QuicSentPacketManager::OnRetransmissionTimeout() {
  // std::cout << "OnRetransmissionTimeout" << std::endl;
...
void QuicSentPacketManager::RetransmitCryptoPackets() {
  // std::cout << "RetransmitCryptoPackets" << std::endl;
...
bool QuicSentPacketManager::MaybeRetransmitTailLossProbe() {
  // std::cout << "MaybeRetransmitTailLossProbe" << std::endl;
  if (pending_timer_transmission_count_ == 0) {
    return false;
  }
  QuicPacketNumber packet_number = unacked_packets_.GetLeastUnacked();
...
void QuicSentPacketManager::RetransmitRtoPackets() {
  // std::cout << "RetransmitRtoPackets" << std::endl;
...
QuicSentPacketManager::GetRetransmissionMode() const {
  // std::cout << "GetRetransmissionMode" << std::endl;
  DCHECK(unacked_packets_.HasInFlightPackets());
...
void QuicSentPacketManager::InvokeLossDetection(QuicTime time) {
  // std::cout << "InvokeLossDetection" << std::endl;
...
bool QuicSentPacketManager::MaybeUpdateRTT(const QuicAckFrame& ack_frame,
                                           QuicTime ack_receive_time) {
  // std::cout << "MaybeUpdateRTT" << std::endl;
...
QuicTime::Delta QuicSentPacketManager::TimeUntilSend(QuicTime now) {
  // std::cout << "TimeUntilSend" << std::endl;
...
const QuicTime QuicSentPacketManager::GetRetransmissionTime() const {
  // std::cout << "GetRetransmissionTime" << std::endl;
...
const QuicTime::Delta QuicSentPacketManager::GetCryptoRetransmissionDelay()
    const {
      // std::cout << "GetCryptoRetransmissionDelay" << std::endl;
...
const QuicTime::Delta QuicSentPacketManager::GetTailLossProbeDelay() const {
  // std::cout << "GetTailLossProbeDelay" << std::endl;
  QuicTime::Delta srtt = rtt_stats_.smoothed_rtt();
  if (srtt.IsZero()) {
    srtt = QuicTime::Delta::FromMicroseconds(rtt_stats_.initial_rtt_us());
  }
...
const QuicTime::Delta QuicSentPacketManager::GetRetransmissionDelay() const {
  // std::cout << "GetRetransmissionDelay" << std::endl;
...
QuicPacketCount QuicSentPacketManager::GetCongestionWindowInTcpMss() const {
  // std::cout << "GetCongestionWindowInTcpMss" << std::endl;
...
// void QuicSentPacketManager::SetInitialCongestionWindow(QuicPacketCount initial_congestion_window){
//   initial_congestion_window_ = initial_congestion_window;
// }
void QuicSentPacketManager::SetSendAlgorithm(
    CongestionControlType congestion_control_type) {
      
  // std::cerr<< kInitialCongestionWindow << std::endl;
...
void QuicSentPacketManager::SetSendAlgorithm(
  ...
  // std::cout << "SetSendAlgorithm SendAlgorithmInterface" << std::endl;
...
void QuicSentPacketManager::OnConnectionMigration(PeerAddressChangeType type) {
  // std::cout << "OnConnectionMigration" << std::endl;
...
SendAlgorithmInterface* QuicSentPacketManager::GetSendAlgorithmNotConst() {
  return send_algorithm_.get();
}

void QuicSentPacketManager::SetStreamNotifier(
    StreamNotifierInterface* stream_notifier) {
  // std::cout << "SetStreamNotifier" << std::endl;
