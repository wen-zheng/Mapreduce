...
void QuicSession::CloseStreamInner(QuicStreamId stream_id, bool locally_reset) {
  ...
  // std::cout << this->GetNumActiveStreams() << std::endl;
  // std::cout << this->IsClosedStream()<< std::endl;
  if(this->perspective() == Perspective::IS_SERVER){

    std::ofstream mapreduceLog( "log.txt", std::ios::app);
    mapreduceLog << this->connection()->peer_address().host().ToString() << ":"
      << this->connection()->GetStats()<<std::endl;
    
    // std::cout << this->connection()->peer_address().host().ToString() << ": "
    //   << this->connection()->GetStats()<<std::endl;
    mapreduceLog.close();
  }
}
