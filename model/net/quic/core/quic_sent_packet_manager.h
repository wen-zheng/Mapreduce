...
class QUIC_EXPORT_PRIVATE QuicSentPacketManager {
 public:
  ...
  RttStats* GetRttStatsNotConst(){ return &rtt_stats_; };
  ...
  // void SetInitialCongestionWindow(QuicPacketCount initial_congestion_window);

  const SendAlgorithmInterface* GetSendAlgorithm() const;

  SendAlgorithmInterface* GetSendAlgorithmNotConst();
